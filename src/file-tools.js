...
export const FILE_TOOL_DEFINITIONS = [
  {
    name: "list_dir",
    description: "List files/directories recursively from a path.",
    args: ["path?", "depth?"],
    details: "Lists files and subdirectories. Default path is workspace root. Depth limits recursion (default: unlimited). Returns a tree of entries with names and types.",
    example: '{"name":"list_dir","arguments":{"path":"src","depth":2}}',
  },
  {
    name: "read_file",
    description: "Read a UTF-8 text file with optional line range.",
    args: ["path", "startLine?", "endLine?"],
    details: "Reads a file and returns its content. If startLine/endLine are given, only that line range is returned (1-indexed). Useful for reading large files in sections.",
    example: '{"name":"read_file","arguments":{"path":"src/index.js","startLine":1,"endLine":50}}',
  },
  {
    name: "write_file",
    description: "Write full file content (creates directories by default).",
    args: ["path", "content", "overwrite?", "createDirs?"],
    details: "Writes content to a file, creating parent directories by default. If overwrite is false and the file exists, returns an error. Use edit_file for partial edits to avoid overwriting unrelated lines.",
    example: '{"name":"write_file","arguments":{"path":"dist/output.txt","content":"hello"}}',
  },
  {
    name: "append_file",
    description: "Append text to a file.",
    args: ["path", "content"],
    details: "Appends text to the end of a file. The file must already exist.",
    example: '{"name":"append_file","arguments":{"path":"log.txt","content":"new entry\\n"}}',
  },
  {
    name: "edit_file",
    description: "Replace first/all occurrences of oldText in a file.",
    args: ["path", "oldText", "newText", "replaceAll?"],
    details: "Performs a string replacement inside a file. oldText must match exactly (including whitespace). If replaceAll is true, all occurrences are replaced; otherwise only the first. Preferred over write_file for surgical edits.",
    example: '{"name":"edit_file","arguments":{"path":"src/app.ts","oldText":"const x = 1;","newText":"const x = 2;"}}',
  },
  {
    name: "delete_path",
    description: "Delete a file or directory (recursive optional).",
    args: ["path", "recursive?"],
    details: "Deletes a file or, if recursive is true, a whole directory tree. Use with care: this is irreversible.",
    example: '{"name":"delete_path","arguments":{"path":"dist","recursive":true}}',
  },
  {
    name: "mkdir",
    description: "Create a directory.",
    args: ["path", "recursive?"],
    details: "Creates a directory. If recursive is true (default), creates all intermediate directories like `mkdir -p`.",
    example: '{"name":"mkdir","arguments":{"path":"src/utils"}}',
  },
  {
    name: "move_path",
    description: "Rename/move a file or directory.",
    args: ["from", "to"],
    details: "Renames or moves a file or directory. Both paths are relative to workspace root. Use this for refactoring file layout without rewriting content.",
    example: '{"name":"move_path","arguments":{"from":"old/file.ts","to":"new/file.ts"}}',
  },
  {
    name: "search_files",
    description: "Search text across files in a directory.",
    args: ["query", "path?", "caseSensitive?", "regex?"],
    details: "Searches all text files under path for query. Supports case-sensitive and regex modes. Returns matching lines with file path and line number.",
    example: '{"name":"search_files","arguments":{"query":"TODO","path":"src","regex":false}}',
  },
  {
    name: "glob_files",
    description: "Find files matching a glob pattern (e.g. **/*.ts, src/*.js).",
    args: ["pattern", "path?"],
    details: "Finds files by name pattern. Supports * (single segment), ** (multiple segments), ? (single char), and {a,b} alternatives. Faster than list_dir + manual filtering.",
    example: '{"name":"glob_files","arguments":{"pattern":"**/*.test.ts","path":"src"}}',
  },
  {
    name: "patch_file",
    description: "Apply a unified-diff patch string to a file.",
    args: ["path", "hunks"],
    details: "Applies a unified-diff patch to a file. The hunks argument should be the body of the diff (starting with @@ lines). Useful for applying complex multi-location changes in one call.",
    example: '{"name":"patch_file","arguments":{"path":"src/app.ts","hunks":"@@ -1,3 +1,4 @@\\n line1\\n+new-line\\n line2\\n line3\\n"}}',
  },
  {
    name: "stat_path",
    description: "Get basic metadata for a file or directory.",
    args: ["path"],
    details: "Returns size, modification time, whether the path is a file or directory. Useful before read_file to check if a file exists or to get its size.",
    example: '{"name":"stat_path","arguments":{"path":"package.json"}}',
  },
  {
    name: "run_command",
    description: "Run a shell command in workspace (requires user approval unless yolo).",
    args: ["command", "cwd?", "timeoutMs?"],
    details: "Executes a shell command inside the workspace. Requires user confirmation unless --yolo is active. cwd is relative to workspace root. Default timeout is 15 seconds. Returns stdout, stderr, and exit code.",
    example: '{"name":"run_command","arguments":{"command":"npm test","cwd":"."}}',
  },
];
